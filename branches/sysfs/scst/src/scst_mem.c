...
int scst_sgv_pools_init(unsigned long mem_hwmark, unsigned long mem_lwmark)
{
	...
	sgv_norm_pool = sgv_pool_create("sgv", sgv_no_clustering, false);
	if (sgv_norm_pool == NULL)
		goto out_err;

	sgv_norm_clust_pool = sgv_pool_create("sgv-clust", sgv_full_clustering,
				false);
	if (sgv_norm_clust_pool == NULL)
		goto out_free_norm;

	sgv_dma_pool = sgv_pool_create("sgv-dma", sgv_no_clustering, false);
